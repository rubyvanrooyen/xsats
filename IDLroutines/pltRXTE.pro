...
openr,2,'RXTE_today' ; i think this is the only change
no_objects=strarr(1)
no_visits=strarr(1)
...
openr,2,'RXTE_today' ; now have to re-read the file back in
;readf,2,no_visits ; 
...
stargap=23 * (double(no_objects-1)^(-0.9))
;stargap=1.73 ; (18) for 50 or more ish
...
; pirint SAST and plot some vertical dashed lines on the hours
    ...
    xyouts,-0.45,maxy+0.2,"NICER",/data,charsize=1.7
...
if SAT_name eq "NICER" then satcol= 250
