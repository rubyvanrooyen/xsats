...
openr,2,'Chan_XMM_Suz_Int_today' ; i think this is the only change
no_objects=strarr(1)
no_visits=strarr(1)
...
openr,2,'Chan_XMM_Suz_Int_today' ; now have to re-read the file back in
;readf,2,no_visits ; 

sizey=double(no_objects)*1.  ;.. ps vert size of whole plot
;if (no_objects le 3) then sizey = 10
...
stargap=12.66 * (double(no_objects-1)^(-0.67))
;stargap=5 ; (4) for 50 or more ish
;if (no_objects le 3) then stargap= 5
...
; pirint SAST and plot some vertical dashed lines on the hours
    ...
    xyouts,-0.2,maxy+0.2,"CHANDRA",/data,charsize=1.7,color=50
    xyouts,-0.3,maxy+0.2,"NUSTAR",/data,charsize=1.7,color=100
    xyouts,-0.35,maxy+0.2,"XMM",/data,charsize=1.7,color=250
    xyouts,-0.1,maxy+0.2,"INTEGRAL",/data,charsize=1.7,color=150
    xyouts,-0.5,maxy+0.1,date,/data,charsize=1.7
...
;endelse
satcol=0
if SAT_name eq "NUSTAR" then satcol= 100
if SAT_name eq "XMM" then satcol= 250
if SAT_name eq "CHANDRA" then satcol= 50
if SAT_name eq "INTEGRAL" then satcol= 160
